...
function renderLeaderPreview() {
  const box = el("div", { class: "leader-preview" }, [
    el("div", { class: "leader-head" }, [
      el("div", { class: "leader-title" }, ["🏆 排行榜"]),
      el("div", { class: "leader-subtitle" }, ["同一位只取最高分，分數相同看時間"]),
    ]),
  ]);
  ...
  if (state.leaderboardError) {
    ...
    list.appendChild(el("div", { class: "subtitle" }, ["目前還沒有排行榜資料"]));
  } else {
    state.leaderboard.slice(0, 10).forEach((r, i) => {
      const rowClass =
        i === 0 ? "leader-row first" :
        i === 1 ? "leader-row second" :
        i === 2 ? "leader-row third" :
        "leader-row";

      list.appendChild(
        el("div", { class: rowClass }, [
          el("div", { class: "rank" }, [i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `#${i + 1}`]),
          el("div", { class: "name-wrap" }, [
            el("div", { class: "name" }, [r.student_name || "（無名）"]),
            el("div", { class: "meta" }, [
              r.best_time_sec != null ? `時間 ${formatSeconds(r.best_time_sec)}` : "尚無時間",
            ]),
          ]),
  ...
  if (state.leaderboard.length > 10) {
    box.appendChild(
      el("div", { class: "leader-more" }, [`共 ${state.leaderboard.length} 筆，點「看排行榜」看全部`])
    );
  }
...
async function openLeaderboardModal() {
  ...
  const modalBodyContent = state.leaderboardError
    ? [el("div", { class: "err" }, [state.leaderboardError])]
    : !state.leaderboard.length
    ? [el("div", { class: "subtitle" }, ["目前還沒有排行榜資料"])]
    : [
        el(
          "div",
          { class: "lb" },
          state.leaderboard.map((r, i) => {
            ...
            return el("div", { class: rowClass }, [
              el("div", { class: "lbRank" }, [i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `#${i + 1}`]),
              el("div", { class: "lbNameWrap" }, [
                el("div", { class: "lbName" }, [r.student_name || "（無名）"]),
                el("div", { class: "lbMeta" }, [
                  r.best_time_sec != null ? `時間 ${formatSeconds(r.best_time_sec)}` : "尚無時間",
                ]),
              ]),
              el("div", { class: "lbScore" }, [String(r.best_score ?? 0)]),
            ]);
          })
        ),
      ];
  ...
  const modal = el("div", { class: "modal" }, [
    ...
    el("div", { class: "modalBody" }, modalBodyContent),
...
async function refreshLeaderboard() {
  ...
  const oldRows = Array.isArray(state.leaderboard) ? [...state.leaderboard] : [];
